...
module.exports = function(self, options) {
  ...
  self.deleteFromTrash = function(req, _id, callback) {
    ...
    self.apos.db.collection('aposAttachments').findOne(criteria)
    .then(function(doc)
    {
      self.apos.attachments.uploadfs.remove('/attachments/' + doc._id + '-' + doc.name + '.' + doc.extension);
      if(doc.group == 'images')
      {
        ['full', 'max', 'one-half', 'one-sixth', 'one-third', 'two-thirds'].forEach(function(size)
        {
          self.apos.attachments.uploadfs.remove('/attachments/' + doc._id + '-' + doc.name + '.' + size + '.' + doc.extension);
        })
      }
    })
    .catch(function(err)
    ...
    [criteria, docCriteria, versionCriteria].forEach(function(qriteria) {
      qriteria = {$and: [qriteria, self.apos.permissions.can(req, 'admin-apostrophe-file')]}
    });
    return callback();
  };

...
var version = require('punycode').version;
...
module.exports = function(self, options) {
    ...
    function body(callback) {
      ...
      function upload(callback) {
        if (image) {
          ...
          {
            self.apos.uploadYT.currentVideo.video = file.path;
          }
  ...
  self.docBeforeSave = function(req, doc, callback) {
    if(doc.attachment && doc.attachment.extension == "mp4" && doc.advisoryLock) {
      var firstVideoCriteria = {
        _id: doc._id,
        description: {$exists: false}
      }
      self.apos.db.collection('aposDocs').findOne(firstVideoCriteria).then(function(foundDoc){
        if (foundDoc) {
          self.apos.uploadYT.currentVideo.title = doc.title;
          self.apos.uploadYT.currentVideo.description = doc.description;
          self.apos.uploadYT.setAuth();
        }
      }) 
    }
  };
  ...
  self.deleteFromTrash = function(req, _id, callback) {
    if (self.apos.permissions.can(req, 'admin-apostrophe-file')) {
      var criteria = {
        trashDocIds: _id,
        trash: true
      }
      var docCriteria = {trash: true, '_id': _id};
      var versionCriteria = {
          "doc._id": _id,
          "$or": [
              {
                  "doc.attachment.trash": {
                      "$ne": true
                  }
              },
              {
                  "doc.attachment.trash": true
              }
          ]
      }
      
      self.db.findOne(criteria)
      .then(function(doc)
      {
        var cb = function(err, data) {
          if (err) {
            self.apos.utils.warn('Fails to remove document ' + _id + ', please check storage');
            return callback(err);
          }
        };
  
        self.uploadfs.remove('attachments/' + doc._id + '-' + doc.name + '.' + doc.extension, cb);
  
        if(doc.group == 'images')
        {
          ['full', 'max', 'one-half', 'one-sixth', 'one-third', 'two-thirds'].forEach(function(size)
          {
            self.uploadfs.remove('attachments/' + doc._id + '-' + doc.name + '.' + size + '.' + doc.extension, cb);
          })
        }
      });
  
      self.db.remove(criteria);
      self.apos.db.collection('aposDocs').remove(docCriteria);
      self.apos.db.collection('aposDocVersions').remove(versionCriteria);  
    }
    return callback(null);
  };
